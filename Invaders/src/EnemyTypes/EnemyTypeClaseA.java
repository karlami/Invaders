...
public class EnemyTypeClaseA implements EnemyType{
    ...
    public EnemyTypeClaseA(double xPos, double yPos, int rows, int columns){
    ...
    public boolean collide(int i, Player player, ListaSimple<EnemyType> enemys) {
       if(enemySprite.isPlay()){
        ...
        for(int w = 0; w < player.playerWeapons.weapons.getSize(); w++){
            if(enemys != null && player.playerWeapons.weapons.get(w).collisionRect(((EnemyTypeClaseA) enemys.get(i)).getRect())){
